...
// ======== MEASURING WINDOW FILTER ========
input bool   UseMeasureFilter        = false;  // Enable/disable measuring window filter
input int    MeasureStartHour        = 4;      // Measuring window start hour
input int    MeasureStartMinute      = 0;      // Measuring window start minute
input int    MeasureEndHour          = 9;      // Measuring window end hour
input int    MeasureEndMinute        = 30;     // Measuring window end minute
input double MeasureThresholdPct     = 0.3;   // Min gain % over window to allow longs (e.g. 0.3 = +0.3%)
...
// ======== MEASURING WINDOW FILTER STATE ========
bool     g_allowLongsNow     = false;  // Gate: true only if last window result passed threshold
bool     g_measureSampled    = false;  // Whether we already sampled this window end
datetime g_lastWindowEnd     = 0;      // Timestamp of last window-end bar we processed
...
// ======== MEASURING WINDOW FILTER FUNCTIONS ========

// Returns midnight (00:00) of the day containing 'ts'
datetime DayStart(datetime ts)
{
   MqlDateTime dt;
   TimeToStruct(ts, dt);
...
// Finds the close of the bar whose open time matches targetHour:targetMinute on dayStart.
double GetBarCloseAtTime(datetime dayStart, int targetHour, int targetMinute)
{
   int targetMinutes = targetHour * 60 + targetMinute;
   int lookback      = 1000; // covers ~3 days on M30, handles weekends
   ...
   for(int i = 1; i <= lookback; i++)
   {
      ...
      datetime barDay = DayStart(t);
      if(barDay < dayStart) break;   // gone past target day, stop
      if(barDay != dayStart) continue;

      MqlDateTime dt;
      TimeToStruct(t, dt);
      int barMinutes = dt.hour * 60 + dt.min;

      if(barMinutes == targetMinutes)
         return iClose(_Symbol, _Period, i);
   }
   return 0.0;
}

// Returns true if barOpen falls strictly inside the measuring window (start inclusive, end exclusive)
bool IsInsideMeasureWindow(datetime barOpen)
{
   if(!UseMeasureFilter) return false;

   MqlDateTime dt;
   TimeToStruct(barOpen, dt);
   int barMinutes   = dt.hour * 60 + dt.min;
   int startMinutes = MeasureStartHour * 60 + MeasureStartMinute;
   int endMinutes   = MeasureEndHour   * 60 + MeasureEndMinute;
...
void CheckMeasureFilter(datetime barOpen)
{
   if(!UseMeasureFilter) return;
   ...
   // Only act on the exact bar that opens at window end
   if(barMinutes != endMinutes) return;

   // Avoid double-sampling the same window-end bar
   if(barOpen == g_lastWindowEnd) return;
   g_lastWindowEnd  = barOpen;
   g_measureSampled = true;

   // Window-end close = bar that just closed (index 1) — confirmed closed bar
   double endClose = iClose(_Symbol, _Period, 1);

   // Window-start close = bar that opened at MeasureStartHour:Minute on the same day
   // For windows spanning midnight, start day may be yesterday
   datetime today     = DayStart(barOpen);
   int      startMins = MeasureStartHour * 60 + MeasureStartMinute;
   int      endMins   = MeasureEndHour   * 60 + MeasureEndMinute;

   datetime startDay  = (startMins < endMins) ? today : today - 86400;
   double   startClose = GetBarCloseAtTime(startDay, MeasureStartHour, MeasureStartMinute);

   if(startClose <= 0.0)
   {
      Print("⚠️ Measure filter: could not find window-start bar at ",
            MeasureStartHour, ":", MeasureStartMinute, " — longs blocked");
      g_allowLongsNow = false;
      return;
   }

   double changePct = (endClose - startClose) / startClose * 100.0;

   if(changePct >= MeasureThresholdPct)
   {
      g_allowLongsNow = true;
      Print("✅ Measure filter PASSED: window change=", DoubleToString(changePct, 3),
            "% >= threshold=", MeasureThresholdPct, "% → longs enabled");
   }
   else
   {
      g_allowLongsNow = false;
      Print("🚫 Measure filter BLOCKED: window change=", DoubleToString(changePct, 3),
            "% < threshold=", MeasureThresholdPct, "% → longs disabled");
   }
}
...
void DisplaySettings()
{
   ...
   // Display measuring window filter settings
   Print("┌────────────────────────────────────────────────────────────┐");
   Print("│ MEASURING WINDOW FILTER                                    │");
   Print("├────────────────────────────────────────────────────────────┤");
   Print("│ Status: ", UseMeasureFilter ? "ENABLED" : "DISABLED");
   if(UseMeasureFilter)
   {
      Print("│ Window start : ", MeasureStartHour, ":", MeasureStartMinute);
      Print("│ Window end   : ", MeasureEndHour,   ":", MeasureEndMinute);
      Print("│ Threshold    : +", MeasureThresholdPct, "%");
   }
   Print("└────────────────────────────────────────────────────────────┘");
...
void OnTick()
{
   ...
   // 🔹 Measuring window filter: evaluate at window end, block entries during window
   CheckMeasureFilter(barOpen);
   ...
   // 🔹 Measuring window gate (ENTRY ONLY)
   //    Block new entries both DURING the window and when last result was negative
   if(UseMeasureFilter)
   {
      if(IsInsideMeasureWindow(barOpen))
      {
         Print("⏱ Inside measuring window → no new entries");
         CancelOldBuyStops();
         return;
      }
      if(!g_allowLongsNow)
      {
         Print("🚫 Measure filter: longs not enabled → no new entries");
         CancelOldBuyStops();
         return;
      }
   }
