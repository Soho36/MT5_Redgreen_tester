...
input int FixedSLPoints     = 5;  // Stop loss distance in whole points (e.g. 5 = 5 index points)
...
{	
   ...
   Print("╚════════════════════════════════════════════════════════════╝");
   Print("Lots: ", Lots,
      ", Fixed TP: ", FixedTPPoints, " points (",
      FixedTPPoints / tickSize, " ticks, $",
      (FixedTPPoints / tickSize) * tickValue * Lots, " per trade)",
      ", Fixed SL: ", FixedSLPoints, " points (",
      FixedSLPoints / tickSize, " ticks, $",
      (FixedSLPoints / tickSize) * tickValue * Lots, " per trade)");
...
void OnTick()
{
   ...
   if(c1 < o1)
   {
      ...
      if(!OrderSend(req, res))
		...
		Print("🚀 BuyStop placed @", entry, " SL=", sl, " TP=", tp,
			  " (TP ", FixedTPPoints, " pts / SL ", FixedSLPoints, " pts, ", weekdayName, ", ", monthName, ")");
   }
}