...
// ======== POSITION SIZING ========
input double MaxLots        = 5.0;    // Max contracts (smallest candle ~10 pts)
input double MinLots        = 1.0;    // Min contracts (largest candle 100+ pts)
input double AnchorSmall    = 10.0;   // Candle size (pts) that gives MaxLots
input double AnchorLarge    = 100.0;  // Candle size (pts) that gives MinLots

input int    FixedTPPoints  = 5;      // Take profit distance in whole points
...
double CalcLots(double candleSizePoints)
{
   // Clamp to anchor range
   double size = MathMax(AnchorSmall, MathMin(candleSizePoints, AnchorLarge));

   // t = 0.0 when size == AnchorSmall (MaxLots)
   // t = 1.0 when size == AnchorLarge (MinLots)
   double t    = (size - AnchorSmall) / (AnchorLarge - AnchorSmall);
   double lots = MaxLots + t * (MinLots - MaxLots);

   // Round to nearest whole contract, clamp to [MinLots, MaxLots]
   double rounded = MathRound(lots);
   return MathMax(MinLots, MathMin(rounded, MaxLots));
}
...
{	
   ...
   Print("║                    EA SETTINGS                             ║");
   Print("╚════════════════════════════════════════════════════════════╝");
   Print("Fixed TP: ", FixedTPPoints, " points (",
      FixedTPPoints / tickSize, " ticks, $",
      (FixedTPPoints / tickSize) * tickValue * MinLots, " per trade @ MinLots)");

   Print("┌────────────────────────────────────────────────────────────┐");
   Print("│ DYNAMIC POSITION SIZING                                    │");
   Print("├────────────────────────────────────────────────────────────┤");
   Print("│ Candle ", DoubleToString(AnchorSmall,1), " pts → ", DoubleToString(MaxLots,0), " contracts (MAX)");
   Print("│ Candle ", DoubleToString(AnchorLarge,1), " pts → ", DoubleToString(MinLots,0), " contract  (MIN)");
   Print("│ Scaling: linear between anchors, rounded to whole contracts│");

   // Print the full sizing table for easy verification
   Print("├────────────────────────────────────────────────────────────┤");
   Print("│ SIZE TABLE:                                                │");
   double step = (AnchorLarge - AnchorSmall) / 9.0;
   for(int i = 0; i <= 9; i++)
   {
      double candleSize = AnchorSmall + i * step;
      double lots       = CalcLots(candleSize);
      Print("│   Candle ~", DoubleToString(candleSize, 1), " pts → ", DoubleToString(lots, 0), " contracts");
   }
   Print("└────────────────────────────────────────────────────────────┘");
...
void OnTick()
{
   ...
   if(c1 < o1)
   {
      // ---- DYNAMIC SIZING BASED ON RED CANDLE SIZE ----
      double candleSizePoints = (h1 - l1);
      double lots             = CalcLots(candleSizePoints);

      Print("🔴 Red candle on ", weekdayName, " in ", monthName,
            " | Candle: ", DoubleToString(candleSizePoints, 1), " pts",
            " → ", DoubleToString(lots, 0), " contracts",
            " → refresh BuyStop");
