...
input double LimitOffset = 5.0;  // Offset below breakout for limit order (in points)
...
// ======== GLOBAL VARIABLES FOR BREAKOUT TRACKING ========
struct BreakoutLevel
{
   double high;      // High of the red candle
   double low;       // Low of the red candle (for stop loss)
   datetime time;    // Time of the red candle
   bool triggered;   // Has the breakout occurred?
   bool orderPlaced; // Has the limit order been placed?
};

BreakoutLevel g_currentBreakout = {0, 0, 0, false, false};
...
// ======== BREAKOUT MONITORING FUNCTIONS ========
bool CheckForBreakout()
{
   // Check if we have an active breakout level to monitor
   if(g_currentBreakout.high == 0 || g_currentBreakout.triggered || g_currentBreakout.orderPlaced)
      return false;
   ...
   // Check if price has broken above the high
   if(currentAsk > g_currentBreakout.high)
   {
      g_currentBreakout.triggered = true;
      Print("🚨 Breakout! Price ", currentAsk, " > red candle high ", g_currentBreakout.high);
      return true;
   }
...
bool PlaceBreakoutLimitOrder()
{
   if(!g_currentBreakout.triggered || g_currentBreakout.orderPlaced)
      return false;

   // Calculate limit price (breakout high minus offset)
   double limitPrice = g_currentBreakout.high - LimitOffset;

   // Get symbol info for normalization
   double point = SymbolInfoDouble(_Symbol, SYMBOL_POINT);
   double tickSize = SymbolInfoDouble(_Symbol, SYMBOL_TRADE_TICK_SIZE);

   // Handle offset conversion (points to price for MNQ)
   double normalizedLimit = limitPrice;

   // Normalize prices
   normalizedLimit = NormalizeDouble(normalizedLimit / tickSize, 0) * tickSize;
   double stop = NormalizeDouble(g_currentBreakout.low / tickSize, 0) * tickSize;

   // Ensure limit price is reasonable (not too far below current price)
   double currentAsk = SymbolInfoDouble(_Symbol, SYMBOL_ASK);
   if(normalizedLimit > currentAsk)
   {
      Print("⚠️ Limit price ", normalizedLimit, " > current ask ", currentAsk,
            " → adjusting to current ask");
      normalizedLimit = NormalizeDouble(currentAsk / tickSize, 0) * tickSize;
   }
   ...
   req.price        = normalizedLimit;
   req.sl           = stop;
   req.deviation    = Slippage;
   req.type_filling = ORDER_FILLING_RETURN;
   req.comment      = "Breakout Pullback";

   if(!OrderSend(req, res))
   {
      Print("❌ Place BuyLimit (breakout) fail err=", GetLastError());
      return false;
   }
   else
   {
      g_currentBreakout.orderPlaced = true;
      Print("🎯 Breakout BuyLimit placed @", normalizedLimit,
            " (", LimitOffset, " points below breakout high ", g_currentBreakout.high,
            ") SL=", stop);
      return true;
   }
}
...
void ResetBreakout()
{
   g_currentBreakout.high = 0;
   g_currentBreakout.low = 0;
   g_currentBreakout.time = 0;
   g_currentBreakout.triggered = false;
   g_currentBreakout.orderPlaced = false;
}
...
void DisplaySettings()
{
   ...
   Print("Limit Offset: ", LimitOffset, " points below breakout");
...
void OnTick()
{
   ...
   // Check for breakout on every tick (not just new bars)
   if(g_currentBreakout.high > 0 && !g_currentBreakout.triggered)
   {
      CheckForBreakout();
   }

   // Place limit order if breakout occurred
   if(g_currentBreakout.triggered && !g_currentBreakout.orderPlaced)
   {
   ...
   if(c1 < o1)
   {
      Print("🔴 Red candle on ", weekdayName, " in ", monthName, " → monitoring breakout above ", h1);

      // Cancel any existing orders
      CancelOldBuyOrders();

      // Set new breakout level
      g_currentBreakout.high = h1;
      g_currentBreakout.low = l1;
      g_currentBreakout.time = barOpen;
      g_currentBreakout.triggered = false;
      g_currentBreakout.orderPlaced = false;

      Print("🎯 Monitoring for breakout above ", h1,
            " (will place buy limit ", LimitOffset, " points below on breakout)");
   }
   else
   {
      // If we get a non-red candle, reset breakout if not already triggered
      if(!g_currentBreakout.triggered && !g_currentBreakout.orderPlaced)
      {
         ResetBreakout();
      }
