...
//+------------------------------------------------------------------+
//| Combined Dynamic Position Sizing (Balance-Step + Risk Based)     |
//+------------------------------------------------------------------+

input bool   UseBalanceStep   = true;
input bool   UseRiskSizing    = true;
input int    MaxContracts     = 10;

// balance-step inputs
input double BaseLots         = 1.0;     // start with 1 contract
input double MinBalance       = 10000.0; // first step threshold
input double BalanceStep      = 5000.0;  // +1 contract every +5k

// risk sizing inputs
input double MinRiskPercent   = 1.0;     // 1% minimum per trade
input double MaxRiskPercent   = 2.0;     // 2% maximum per trade

//-------------------- Helpers --------------------------//
int GetFinalLotSize(double entryPrice, double stopPrice)
{
   double balance = AccountInfoDouble(ACCOUNT_BALANCE);

   // ---------------------------
   // 1) Balance-step sizing
   // ---------------------------
   int balanceLots = (int)BaseLots;
   ...
   if (UseBalanceStep)
   {
      if (balance > MinBalance)
      {
         ...
         balanceLots = (int)(BaseLots + steps);
      }
   }

   if (balanceLots < 1) balanceLots = 1;


   // ---------------------------
   // 2) Risk-based sizing
   // ---------------------------
   int riskLots = MaxContracts;
   ...
   if (UseRiskSizing)
   {
      ...
      if (tickValue <= 0.0 || tickSize <= 0.0)
		{
		   Print("⚠️ Tick parameters missing → risk sizing disabled for this symbol.");
		   riskLots = MaxContracts;  // fallback
		}
		else
		{
		   double riskPoints = MathAbs(entryPrice - stopPrice);
		   double riskPerContract = riskPoints * (tickValue / tickSize);

		   double minRiskMoney = balance * (MinRiskPercent / 100.0);
		   double maxRiskMoney = balance * (MaxRiskPercent / 100.0);

         if (riskPerContract > maxRiskMoney)
         {
            Print("⚠️ Skip trade: 1 contract risk exceeds max allowed.");
            return 0;
         }

         if (riskPerContract < minRiskMoney)
         {
            Print("⚠️ Skip trade: risk too small for minimum threshold.");
            return 0;
         }

         int maxRiskBased = (int)MathFloor(maxRiskMoney / riskPerContract);
         if (maxRiskBased < 1) maxRiskBased = 1;

         riskLots = maxRiskBased;
      }
   }
   ...
   // 3) Combined sizing
   // ---------------------------
   int finalLots = balanceLots;
   if (UseRiskSizing)
      finalLots = MathMin(finalLots, riskLots);

   finalLots = MathMin(finalLots, MaxContracts);
   if (finalLots < 1) finalLots = 1;

   PrintFormat("📊 Sizing → BalanceLots=%d | RiskLots=%d | Final=%d | Balance=%.2f",
               balanceLots, riskLots, finalLots, balance);
...
void OnTick()
{
   ...
   if(c1 < o1)
   {
       ...
       int finalLots = GetFinalLotSize(entry, stop);
	   if(finalLots == 0)
   {
	   // sizing decided to skip the trade (too large risk or too small risk)
	   Print("⛔ Trade skipped by sizing rules (entry=", entry, " stop=", stop, ")");
	   return; // if you want to continue scanning next bars instead of returning, use `continue` in loop context;
   }
	   ...
	   req.symbol       = _Symbol;
	   req.volume       = (double)finalLots;          // use final integer contracts
	   ...
	   if(!OrderSend(req, res))
		  Print("❌ Place BuyStop fail err=", GetLastError(), " entry=", entry, " vol=", finalLots);
	   else
		  Print("🚀 BuyStop placed @", entry, " SL=", stop, " vol=", finalLots);
   }
}