//+------------------------------------------------------------------+
//| Red-Green Breakout EA: dynamic exit + time filter                |
//+------------------------------------------------------------------+
#property strict
...
void PlaceScaleInLimits()
{
   if(!UseScaleInLimit || g_signalHigh <= 0.0)
      return;

   double range = g_signalHigh - g_signalLow;
   if(range <= 0.0)
   {
      Print("⚠️ Invalid setup range");
      return;
   }

   if(!UseLimit1)
   {
      Print("⚠️ Limit1 is disabled, nothing to place");
   ...
   double pct        = MathMax(0.0, MathMin(100.0, LimitOffsetPercent1));
   ...
   {
      Print("⚠️ BuyLimit 1 too close -> skipped");
      g_limitPlacedAfterEntry = true;
      return;
   }
   ...
   if(!OrderSend(req, res))
      Print("❌ BuyLimit 1 failed err=", GetLastError());
   else
      Print("✅ BuyLimit 1 placed @ ", limitPrice);
...
void ManageOpenPosition()
{
   ...
   {
      Print("⚠️ Initial reference missing");
      return;
   }
   ...
   if(barClose >= target)
   {
      ...
      if(!OrderSend(req, res))
         ...
         Print("✅ Closed at fixed R:R target");
   }
   else
   {
      Print("⏳ Waiting target=", target);
   }
}
...
void DisplaySettings()
{
   ...
   Print("LimitLots1: ", LimitLots1, ", LimitOffsetPercent1: ", LimitOffsetPercent1);
...
{
   // =====================================================
   // FEATURE OFF -> remove pending limits
   // =====================================================
   if(!UseScaleInLimit)
   {
      CancelAllBuyLimits();
   ...
   {
      // TP reference = breakout candle high
      g_initialEntry = g_signalHigh;
      g_initialRisk  = g_signalHigh - g_signalLow;
      g_initialSet   = (g_initialRisk > 0.0);

      Print("📌 Position opened. Ref entry=", g_initialEntry,
            " risk=", g_initialRisk);
   }
   ...
   // =====================================================
   // INTRABAR BREAKOUT -> PLACE LIMITS ONLY
   // =====================================================
   if(UseScaleInLimit &&
      !isInPosition &&
      !g_breakoutTriggered &&
      g_signalHigh > 0.0)
   {
      ...
      if(ask >= g_signalHigh)
      {
         Print("⚡ Breakout detected -> placing BuyLimits");
         PlaceScaleInLimits();
   ...
   {
      Print("⚠️ Candle range rejected");
      return;
   }

	// =====================================================
	// RED CANDLE -> ARM SETUP (NO BUY STOP)
	// Cancel any old scale-in limits so only the latest setup stays active.
	// =====================================================
	if(c1 < o1)
	{
	   double risk = h1 - l1;
	   ...
	   Print("🔴 Red candle setup armed. Previous BuyLimits cleared. High=", h1,
			 " Low=", l1);
	}
}