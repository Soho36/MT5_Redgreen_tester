...
void OnTick()
{
   ...
   // 🔹 setup (MODIFIED: Now checks for GREEN candle instead of RED)
   ...
   // CHANGED FROM: if(c < o) // Red candle
   // CHANGED TO: if(c > o) // Green candle
   if(c > o)  // Now placing buy orders above GREEN candles
   {
      ...
      r.price        = h;      // Entry above the green candle
      r.sl           = l;      // Stop loss below the green candle
      r.deviation    = Slippage;
      ...
      if(!OrderSend(r, s))
         ...
         Print("🚀 BuyStop placed @", h, " SL=", l, " (Above Green Candle)");
   }
}