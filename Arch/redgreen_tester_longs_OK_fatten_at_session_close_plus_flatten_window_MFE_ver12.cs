...
//---------------- Trade mgmt ----------------//
void ManageOpenPosition(double entry, double sl, double vol)
{
...
void OnTick()
{
   ...
   if(PositionSelect(_Symbol))
   {
      ...
      if(!g_tracking)
      {
		...
		g_slPrice    = PositionGetDouble(POSITION_SL);
        g_entryTime  = (datetime)PositionGetInteger(POSITION_TIME);
		double tickSize  = SymbolInfoDouble(_Symbol, SYMBOL_TRADE_TICK_SIZE);
		double tickValue = SymbolInfoDouble(_Symbol, SYMBOL_TRADE_TICK_VALUE);
		double vol       = PositionGetDouble(POSITION_VOLUME);

		double slPoints = MathAbs(g_entryPrice - g_slPrice);

		if(tickSize > 0)
		   g_slMoney = slPoints / tickSize * tickValue * vol;
		else
		   g_slMoney = 0.0;
   ...
   if(PositionsTotal() > 0)
   {
		...
		ManageOpenPosition(entry, sl, vol);
    return;
   }
