...
input double RR0000W0100 = 1.0;   // R/R for 00:00-01:00
...
double windowRiskRewards[26] =
{
   RR0000W0100,
   RR0100W0130, RR0130W0200,
   RR0200W0300, RR0300W0400, RR0400W0500, RR0500W0600,
   RR0600W0700, RR0700W0800, RR0800W0900, RR0900W1000,
   RR1000W1100, RR1100W1200, RR1200W1300, RR1300W1400,
   RR1400W1500, RR1500W1600, RR1600W1700, RR1700W1800,
   RR1800W1900, RR1900W2000, RR2000W2100, RR2100W2200,
   RR2200W2300,
   RR2300W2330, RR2330W0000
};
...
double g_pendingRiskReward  = 0.0;
double g_positionRiskReward = 0.0;
...
int GetTimeWindowSlot(datetime timeValue)
{
   MqlDateTime dt;
...
bool IsTradeWindow(datetime barOpen)
{
   ...
   return windows[GetTimeWindowSlot(barOpen)];
}

double GetWindowRiskReward(datetime timeValue)
{
   return windowRiskRewards[GetTimeWindowSlot(timeValue)];
}

string BuildRiskRewardComment(double riskReward)
{
   return "WRR=" + DoubleToString(riskReward, 8);
}

double ReadRiskRewardComment(string comment)
{
   int marker = StringFind(comment, "WRR=");
   if(marker < 0)
      return 0.0;

   return StringToDouble(StringSubstr(comment, marker + 4));
}
...
void ManageOpenPosition()
{
   ...
   if(g_positionRiskReward <= 0.0)
   {
      Print("Invalid locked window R:R - skipping TP logic");
      return;
   }

   double target = g_initialEntry + g_initialRisk * g_positionRiskReward;
...
void DisplaySettings()
{
   ...
   Print("║                    EA SETTINGS                             ║");
   Print("╚════════════════════════════════════════════════════════════╝");
   Print("Lots: ", Lots, ", RiskReward: configured separately for each time window");
...
int OnInit()
{
   ...
   {
      if(windowRiskRewards[slot] <= 0.0)
      {
         Print("Invalid R/R for time-window slot ", slot, ": ", windowRiskRewards[slot]);
   ...
   datetime barOpen = iTime(_Symbol, _Period, 0);
	...
	if(isInPosition && !g_wasInPosition)
	{
	   ...
	   if(sl > 0.0 && entry > sl)
	   {
		  ...
		  g_positionRiskReward = ReadRiskRewardComment(PositionGetString(POSITION_COMMENT));
		  if(g_positionRiskReward <= 0.0)
			 g_positionRiskReward = g_pendingRiskReward;
		  if(g_positionRiskReward <= 0.0)
		  {
			 datetime positionTime = (datetime)PositionGetInteger(POSITION_TIME);
			 g_positionRiskReward = GetWindowRiskReward(positionTime);
			 Print("Position comment had no window R:R; using position-time fallback");
		  }
		  g_initialSet   = true;

		  Print("📌 Initial trade locked: Entry=", g_initialEntry,
				" Risk=", g_initialRisk, " Window R:R=", g_positionRiskReward);
	   }
	   else
	   {
	   ...
	   else
	   {
		  ...
		  for(int i = 0; i < 4; i++)
		  {
			  if(!enabled[i])
		   {
			 ...
			 {
				Print("⚠️ Limit ", i+1, " too close or above bid (@", limitPrice, ") → skipping");
				continue;
			 }
			 ...
			 req.comment      = BuildRiskRewardComment(g_positionRiskReward);

			 if(!OrderSend(req, res))
				Print("❌ BuyLimit ", i+1, " failed err=", GetLastError());
			 else
				Print("✅ BuyLimit ", i+1, " placed @", limitPrice, " lots=", lots[i]);
		  }
	...
	if(UseScaleInLimit && !isInPosition && g_wasInPosition)
	{
	   ...
	   g_initialRisk  = 0.0;
	   g_initialSet   = false;
	   g_pendingRiskReward  = 0.0;
	   g_positionRiskReward = 0.0;
	}
   ...
   if(c1 > o1)  // ← GREEN candle condition (was: c1 < o1 for red)
	{
	   ...
	   g_pendingRiskReward = GetWindowRiskReward(barOpen);
	   if(g_pendingRiskReward <= 0.0)
	   {
		  Print("Invalid R/R for current time window - skipping signal");
		  return;
	   }
	   ...
	   req.deviation    = Slippage;
	   req.type_filling = ORDER_FILLING_RETURN;
	   req.comment      = BuildRiskRewardComment(g_pendingRiskReward);

	   if(!OrderSend(req, res))
		  Print("❌ Place BuyStop fail err=", GetLastError());
	   else
		  Print("🚀 BuyStop placed @", entry, " Window R:R=", g_pendingRiskReward);
	}
}
