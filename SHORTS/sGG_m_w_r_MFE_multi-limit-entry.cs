//+------------------------------------------------------------------+
//| Red Breakout EA: dynamic exit + time + month + weekday filters  |
//+------------------------------------------------------------------+
#property strict
...
void OnTick()
{
   ...
   if(UseScaleInLimit && isInPosition && !g_wasInPosition && !g_limitPlacedAfterEntry && g_signalLow > 0)
   {
      ...
      else
      {
         ...
         for(int i = 0; i < 4; i++)
         {
            ...
            // For shorts: limit prices are ABOVE entry (stepping toward SL=signalHigh)
   ...
   // SELL-STOP ORDER AT PREVIOUS RED CANDLE LOW
   if(c1 < o1)  // ← RED candle condition
   {
      Print("🔴 Red candle → place SellStop");
